/*************************************************************************
	> File Name: 1814.cpp
	> Author: Xeon
	> Created Time: 2017年04月19日 星期三 15时43分22秒
 ************************************************************************/
...
{
	int ret=0;
	for (int i=start;i<=6;i++) ret=(ret*10%prime+a[i])%prime;
	for (int i=1;i<start;i++) ret=(ret*10%prime+a[i])%prime;
	return ret;
}
...
{
	int ret=0;
	for (int i=start;i<=6;i++) ret=(ret*13%prime+a[i])%prime;
	for (int i=1;i<start;i++) ret=(ret*13%prime+a[i])%prime;
	return ret;
}
...
{
	int ret=0;
	for (int i=end;i>=1;i--) ret=(ret*10%prime+a[i])%prime;
	for (int i=6;i>end;i--) ret=(ret*10%prime+a[i])%prime;
	return ret;
}
...
{
	int ret=0;
	for (int i=end;i>=1;i--) ret=(ret*13%prime+a[i])%prime;
	for (int i=6;i>end;i--) ret=(ret*13%prime+a[i])%prime;
	return ret;
}
